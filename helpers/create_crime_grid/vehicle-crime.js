	    ...
	    for (var i=0; i<uk_crime_grid.geometry.crimes.length; i++){
	      if(uk_crime_grid.geometry.crimes[i].length > 0){
	        for (var j=0; j<uk_crime_grid.geometry.crimes[i].length; j++){
	        	if (uk_crime_grid.geometry.crimes[i][j].category === 'vehicle-crime'){
	               counter++;
	        	}	
	        }
	        if (counter > 0) {
	          vehicle_crime_heatmap.push({location: uk_crime_grid.geometry.coordinates[i], weight: counter});
	        }
	        counter = 0;
	      } 
	    }
	    fs.writeFileSync(path.join(__dirname,'vehicle_crime_heatmap.json'), JSON.stringify(vehicle_crime_heatmap, null, 2) , 'utf-8');
	  });
	}
}());